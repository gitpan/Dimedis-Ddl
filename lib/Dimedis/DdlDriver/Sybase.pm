...
package Dimedis::Ddl::PrimaryKey::Sybase;
use vars qw ( @ISA );
...
package Dimedis::Ddl::Table::Sybase;
use vars qw ( @ISA );
...
package Dimedis::Ddl::Column::Sybase;
use vars qw ( @ISA );
...
sub get_ddl {
	...
	if ( $type eq 'serial' ) {
		...
		$full_type =~ /varchar\s*\((\d+)\)/;
		if ( $1 > 255 ) {
			if ( $self->get_like_search ) {
				$sql .= "VARCHAR(255) ";
			} else {
				$sql .= "TEXT ";
			}
...
package Dimedis::Ddl::Index::Sybase;
use vars qw ( @ISA );
@ISA = qw ( Dimedis::Ddl::Index );

package Dimedis::Ddl::Constraint::Sybase;
use vars qw ( @ISA );
...
package Dimedis::Ddl::Reference::Sybase;
use vars qw ( @ISA );
